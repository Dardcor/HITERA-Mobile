...
class SettingsProvider extends ChangeNotifier {
  ...
  bool get lokasiEnabled => _lokasiEnabled;
  ...
  Future<void> loadSettings(String userId) async {
    ...
    try {
      ...
      if (data != null) {
        ...
        _lokasiEnabled = data['lokasi_enabled'] ?? false;
  ...
  Future<void> _rescheduleAllAlarms() async {
    if (_keuanganNotifEnabled && _keuanganNotifTime != null) {
      final t = _keuanganNotifTime!.split(':');
      await NotificationService.scheduleDailyNotification(
        id: 1, title: 'Pengingat Keuangan', body: 'Jangan lupa catat pemasukan dan pengeluaran hari ini! 💰',
    ...
    if (_kesehatanNotifEnabled && _kesehatanNotifTime != null) {
      final t = _kesehatanNotifTime!.split(':');
      await NotificationService.scheduleDailyNotification(
        id: 2, title: 'Pengingat Kesehatan', body: 'Sudah catat data kesehatan hari ini? 💪',
        time: TimeOfDay(hour: int.parse(t[0]), minute: int.parse(t[1])),
      );
    } else {
    ...
    if (_tugasNotifEnabled && _tugasNotifTime != null) {
      final t = _tugasNotifTime!.split(':');
      await NotificationService.scheduleDailyNotification(
        id: 3, title: 'Pengingat Tugas', body: 'Cek daftar tugas Anda hari ini! ✅',
        time: TimeOfDay(hour: int.parse(t[0]), minute: int.parse(t[1])),
      );
    } else {
  ...
  Future<void> updateLokasi(String userId, bool value) async {
    _lokasiEnabled = value;
    notifyListeners();
    try {
      await SupabaseService.updateUserSettings(userId, {'lokasi_enabled': value});
    } catch (e) {
      debugPrint('Error updating lokasi: $e');
      _lokasiEnabled = !value;
      notifyListeners();
    }
  }
